...
func (rq *ResourceQuery) QueryPods() *PodQuery {
	query := (&PodClient{config: rq.config}).Query()
...
func (rq *ResourceQuery) WithPods(opts ...func(*PodQuery)) *ResourceQuery {
	query := (&PodClient{config: rq.config}).Query()
...
func (rq *ResourceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Resource, error) {
	...
	if query := rq.withPods; query != nil {
		if err := rq.loadPods(ctx, query, nodes,
			func(n *Resource) { n.Edges.Pods = []*Pod{} },
			func(n *Resource, e *Pod) { n.Edges.Pods = append(n.Edges.Pods, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ResourceQuery) loadPods(ctx context.Context, query *PodQuery, nodes []*Resource, init func(*Resource), assign func(*Resource, *Pod)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(resource.PodsTable)
		s.Join(joinT).On(s.C(pod.FieldID), joinT.C(resource.PodsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(resource.PodsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(resource.PodsPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "pods" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}

...
func (ru *ResourceUpdate) SetNodeName(s string) *ResourceUpdate {
	ru.mutation.SetNodeName(s)
	return ru
}

// SetNillableNodeName sets the "node_name" field if the given value is not nil.
func (ru *ResourceUpdate) SetNillableNodeName(s *string) *ResourceUpdate {
	if s != nil {
		ru.SetNodeName(*s)
	}
	return ru
...
func (ru *ResourceUpdate) AddPodIDs(ids ...uuid.UUID) *ResourceUpdate {
	ru.mutation.AddPodIDs(ids...)
	return ru
}

// AddPods adds the "pods" edges to the Pod entity.
func (ru *ResourceUpdate) AddPods(p ...*Pod) *ResourceUpdate {
	ids := make([]uuid.UUID, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return ru.AddPodIDs(ids...)
}
...
func (ru *ResourceUpdate) ClearPods() *ResourceUpdate {
	ru.mutation.ClearPods()
	return ru
}

// RemovePodIDs removes the "pods" edge to Pod entities by IDs.
func (ru *ResourceUpdate) RemovePodIDs(ids ...uuid.UUID) *ResourceUpdate {
	ru.mutation.RemovePodIDs(ids...)
	return ru
}

// RemovePods removes "pods" edges to Pod entities.
func (ru *ResourceUpdate) RemovePods(p ...*Pod) *ResourceUpdate {
	ids := make([]uuid.UUID, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return ru.RemovePodIDs(ids...)
}
...
func (ru *ResourceUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if value, ok := ru.mutation.NodeName(); ok {
		_spec.SetField(resource.FieldNodeName, field.TypeString, value)
	}
	if ru.mutation.PodsCleared() {
	...
	if nodes := ru.mutation.RemovedPodsIDs(); len(nodes) > 0 && !ru.mutation.PodsCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   resource.PodsTable,
	...
	if nodes := ru.mutation.PodsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   resource.PodsTable,
...
// SetNodeName sets the "node_name" field.
func (ruo *ResourceUpdateOne) SetNodeName(s string) *ResourceUpdateOne {
	ruo.mutation.SetNodeName(s)
	return ruo
}

// SetNillableNodeName sets the "node_name" field if the given value is not nil.
func (ruo *ResourceUpdateOne) SetNillableNodeName(s *string) *ResourceUpdateOne {
	if s != nil {
		ruo.SetNodeName(*s)
	}
	return ruo
...
// AddPodIDs adds the "pods" edge to the Pod entity by IDs.
func (ruo *ResourceUpdateOne) AddPodIDs(ids ...uuid.UUID) *ResourceUpdateOne {
	ruo.mutation.AddPodIDs(ids...)
	return ruo
}

// AddPods adds the "pods" edges to the Pod entity.
func (ruo *ResourceUpdateOne) AddPods(p ...*Pod) *ResourceUpdateOne {
	ids := make([]uuid.UUID, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return ruo.AddPodIDs(ids...)
}
...
// ClearPods clears all "pods" edges to the Pod entity.
func (ruo *ResourceUpdateOne) ClearPods() *ResourceUpdateOne {
	ruo.mutation.ClearPods()
	return ruo
}

// RemovePodIDs removes the "pods" edge to Pod entities by IDs.
func (ruo *ResourceUpdateOne) RemovePodIDs(ids ...uuid.UUID) *ResourceUpdateOne {
	ruo.mutation.RemovePodIDs(ids...)
	return ruo
}

// RemovePods removes "pods" edges to Pod entities.
func (ruo *ResourceUpdateOne) RemovePods(p ...*Pod) *ResourceUpdateOne {
	ids := make([]uuid.UUID, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return ruo.RemovePodIDs(ids...)
}
...
func (ruo *ResourceUpdateOne) sqlSave(ctx context.Context) (_node *Resource, err error) {
	...
	if value, ok := ruo.mutation.NodeName(); ok {
		_spec.SetField(resource.FieldNodeName, field.TypeString, value)
	}
	if ruo.mutation.PodsCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   resource.PodsTable,
	...
	if nodes := ruo.mutation.RemovedPodsIDs(); len(nodes) > 0 && !ruo.mutation.PodsCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   resource.PodsTable,
	...
	if nodes := ruo.mutation.PodsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   resource.PodsTable,
